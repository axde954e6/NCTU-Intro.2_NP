...
class ServerService(object):
    def __init__(self):
        """
        self.Session = {} #session id -> user_name
        self.user_password = {} #user_name -> password
        self.user_email = {}    #user_name -> email
        self.board = {} #board_name -> {"Moderator","have_post_SN":[]}
        self.post = {}  #post_serial_number -> {"title",content","Author","Date","comment":{username:comment},"board_name"}
        """
        ...
        if cmd == "INITIAL" and sessionID == -1:
            ...
            if len(Arg) != 2:
                ...
                if user_info.Session[sessionID] == "":
                    ...
                    user_info.Session[sessionID]
                    != user_info.post[int(Arg[1])]["Author"]
                ):
            ...
            if len(Arg) != 3:
                ...
                if user_info.Session[sessionID] == "":
                    ...
                    user_info.post[int(Arg[1])]["comment"].append(
                        {"username": user_info.Session[sessionID], "comment": Arg[2]})
